...
│   ├── safety_info.py                    참고 치안정보 — CCTV 표준데이터 조회
│   └── registry_parser.py                등기부 PDF → OpenAI로 권리관계 자동 추출 (--registry-pdf)
│
...
기본 대상 건물은 수원시 팔달구 우만동 39-7 (직방 리포트로 실측 검증한 그 건물). `--my-deposit 5000000 --mortgage 0`은 지어낸 값이 아니라 **실제 이 건물에 500/50 월세로 거주 중인 팀원의 실계약(보증금 500만원) + 실제 등기사항전부증명서로 확인한 근저당(0원)** 이다 — 등기부 열람은 임대인 동의 없이 누구나 열람 700원/발급 1,000원이면 되므로([iros.go.kr](https://www.iros.go.kr)), 계약 전에 직접 떼서 이 값을 넣으면 된다. 다른 건물을 보려면 `--sigungu --bjdong --bun --ji --dong-name`을 바꾸면 된다.

### 리포트를 어떻게 읽고 쓰면 되나 (실사용 시나리오)

위 명령을 실제로 돌리면 이런 순서로 판단하면 된다:

1. **"총 가구수(N)"과 "숨은 선순위 임차보증금"** — 다가구면 여기가 핵심. "9가구 중 나 말고 8세대가 걸어둔 보증금이 평균 얼마쯤 될 것 같다"는 확률적 추정치다. 등기부엔 안 보이는 돈이라 이 숫자가 전세사기 리포트의 존재 이유.
2. **"실질 부채비율"** — 근저당(등기부로 실측) + 위 선순위 추정치를 시세로 나눈 값. 이게 높을수록(특히 80% 이상) 위험 구간.
3. **"사고 발생 확률"** — 논문 기반 근사치. 4~5%대면 낮은 편, 15% 넘어가면 신중하게 봐야 함. **건물 자체의 위험도라 내 보증금 액수와 무관하게 동일**하다.
4. **"보증금 회수 예상" / "기대손실"** — 여기가 내 상황(월세 500만원 vs 전세 5천만원 등)에 따라 달라지는 부분. 절대 금액(원)으로 나오니 "이 정도 손실 가능성이면 계약해도 괜찮은가"를 스스로 판단하면 된다.
5. **확정일자 부여현황을 이미 받아본 상태라면** `--known-tenants`/`--known-priority-deposit-won`으로 그만큼은 추정이 아니라 확정값으로 반영해서 다시 돌려보면 더 정확해진다.
6. **국세·지방세 체납이 확인됐다면** `--tax-arrears-won`으로 반영 — 당해세는 근저당보다도 우선순위가 높을 수 있어서 회수액에서 우선 차감된다.
7. **등기부 PDF가 있다면 `--mortgage` 대신 `--registry-pdf 파일1.pdf 파일2.pdf`** — 숫자를 직접 안 읽어도 OpenAI가 근저당·압류·가압류·신탁·소유권이전 이력을 자동으로 뽑아준다(이름·주민번호는 전송 전 자동 제거). 다만 말소(취소선) 서식은 텍스트로는 구분이 안 되니 원본을 육안으로도 한 번 확인하는 걸 권장.
8. **`--json`을 붙이면** 위 내용이 그대로 구조화된 JSON으로 나와서 프론트엔드/다른 시스템에 연결하기 쉽다.

한마디로: **등기부(근저당)는 직접 떼서 넣거나 PDF로 자동추출하고, 나머지(선순위 보증금·사고확률·기대손실)는 이 도구가 계산해주는 값을 참고해서 계약 여부를 판단**하면 된다.
...
| `--mortgage` | `--registry-pdf`와 둘 중 하나 필수 | — | 근저당 금액 (원) — 등기부 직접 확인 필요 (위 한계표 참고) |
| `--registry-pdf` | `--mortgage`와 둘 중 하나 필수 | — | 등기사항전부증명서 PDF 경로(여러 장 가능, 예: 토지+건물). OpenAI가 근저당·압류·가압류·신탁·소유권이전이력을 자동 추출해 `--mortgage` 대신 사용. 이름·주민번호는 전송 전 레닥션됨. 말소(취소선) 서식은 인식 못하니 원본 육안 대조 권장 |
...
```jsonc
{
  ...
  "module_c": {                          // [C] 사고확률 스코어링
    ...
    "has_attachment": false,             // 압류 (있으면 위험배수 ×5, 논문 아닌 휴리스틱)
    "has_provisional_attachment": false, // 가압류 (있으면 위험배수 ×5)
    "has_trust": false                   // 신탁 (있으면 위험배수 ×2)
  },
  ...
  "registry_extraction": null,           // --registry-pdf 사용 시 PDF별 LLM 추출 원본 배열, 아니면 null
  ...
  "inputs_used": {                       // 사용자가 넣은 값 + 시세 출처 기록(재현성/감사용)
    ...
    "mortgage_won": 0,
    "mortgage_source": "manual",         // "manual" 또는 "registry_pdf"
...
- ✅ **등기부 PDF 자동분석(`--registry-pdf`) 완료** — `src/registry_parser.py` 신규. 이름·주민번호를 정규식으로 레닥션한 뒤 OpenAI로 근저당·압류·가압류·신탁·소유권이전이력을 구조화 추출해 `--mortgage` 수동 입력을 대체. 압류·가압류·신탁 여부는 C모듈 위험배수에도 반영(휴리스틱). 실제 등기부 PDF 2건(토지·건물)으로 검증 완료 — 말소(취소선) 서식은 텍스트 추출로 구분 안 되는 한계는 리포트에 명시
